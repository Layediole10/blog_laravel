              ...
              <td>
                <div class="form-check form-switch">
                  <input class="form-check-input" onchange="if(confirm('Are you sure to change the state of this user?')){
                    document.getElementById('publish-{{$user->id}}').submit();
                    }" type="checkbox" @if ($user->activate)
                      checked
                    @endif name="publish"  role="switch" id="publish">
                </div>
                <form id="publish-{{$user->id}}" action="" method="post">
                  @csrf
                  @method('put')
                </form> 
              </td>
              <td>
                @if ($user->photo)
                    ...
                    <a href="{{route('users.show', ['user'=>$user->id])}}">
                        <img src="{{$user->photo}}" alt="{{$user->name}}" width="50px">
                    </a>    
                        @else
                          <a href="{{route('users.show', ['user'=>$user->id])}}">
                            <img src="{{asset('storage/'.$user->photo)}}" alt="{{$user->name}}" width="50px">
                          </a>
                      @endif
                    @else
                      <a href="{{route('users.show', ['user'=>$user->id])}}">
                        <img src="{{asset('storage/imgUser/avatar.png')}}" alt="{{$user->name}}" width="50px">
                      </a>
                @endif
